...
#define dynarray_reserve2(arr, items_field_1, items_field_2, reserve_count) \
    do { \
        usize dynarray_reserve2__new_capacity = (arr).count + reserve_count; \
        if ((arr).capacity < dynarray_reserve_2__new_capacity) { \
            (arr).capacity = dynarray_reserve_2__new_capacity; \
...
#define dynarray_reserve2_in(arr, items_field_1, items_field_2, reserve_count) \
    do { \
        ...
        if ((arr).capacity < dynarray_reserve_2__new_capacity) { \
            (arr).capacity = dynarray_reserve_2__new_capacity; \
            (arr)items_field = just_realloc_array((arr).allocator, /*Type*/((arr)items_field_1[0]), /*ptr*/((arr)items_field_1), /*count*/((arr).capacity)); \
            (arr)items_field = just_realloc_array((arr).allocator, /*Type*/((arr)items_field_2[0]), /*ptr*/((arr)items_field_2), /*count*/((arr).capacity)); \
        } \
    } while(0)
...
#define dynarray_insert(arr, items_field, index, item) \
    do { \
...
#define dynarray_insert_in(arr, items_field, index, item) \
    do { \
        usize dynarray_insert__index = MIN(index, (arr).count); \
...
#define dynarray_swap_remove(arr, items_field, index) \
    do { \
        (arr)items_field[index] = (arr)items_field[(arr).count - 1]; \
        (arr).count--; \
    } while(0)

#define dynarray_swap_remove2(arr, items_field_1, items_field_2, index) \
    do { \
...
#define dynarray_clone(dst_arr, src_arr, items_field) \
    do { \
        usize dynarray_clone__size = sizeof((src_arr)items_field[0]) * (src_arr).count; \
        (dst_arr).count = (src_arr).count; \
        (dst_arr).capacity = (src_arr).count; \
        (dst_arr)items_field = (src_arr)items_field; \
        (dst_arr)items_field = std_malloc(dynarray_clone__size); \
        std_memcpy((dst_arr)items_field, (src_arr)items_field, dynarray_clone__size); \
    } while(0)
